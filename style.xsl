...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="bldg:Building[bldg:lod0FootPrint][bldg:measuredHeight][bldg:class]">
        <xsl:apply-templates select="bldg:lod0FootPrint//gml:Polygon">
            <xsl:with-param name="z" select="number(bldg:measuredHeight)" />
            <xsl:with-param name="id" select="string(bldg:class)" />
        </xsl:apply-templates>
    </xsl:template>
    
    
    <xsl:template match="bldg:Building[bldg:lod0RoofEdge][bldg:measuredHeight][bldg:class]">
